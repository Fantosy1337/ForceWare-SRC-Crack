...
public class Slider extends Component
{
    ...
    public void renderComponent() {
        ...
        Minecraft.func_71410_x().field_71466_p.func_175063_a(this.set.getName() + ": " + this.set.getValDouble(), (float)this.parent.parent.getX(), (float)(this.parent.parent.getY() + this.offset + 2), -1);
    }
    ...
    public void updateComponent(final int mouseX, final int mouseY) {
        ...
        final double diff = Math.min(76, Math.max(0, mouseX - this.x));
        final double min = this.set.getMin();
        final double max = this.set.getMax();
        this.renderWidth = 76.0 * (this.set.getValDouble() - min) / (max - min);
        if (this.dragging) {
            if (diff == 0.0) {
